...
class AwsBuilderConfig(BuilderConfig):
        ...
        if self.__directory is None:
            if "directory" in self.target_config:
                ...
                if not isinstance(directory, str):
                    raise TypeError(
                        f"Field `tool.hatch.build.targets.{self.plugin_name}.directory` " "must be a string"
                    )
            else:
                directory = self.build_config.get("directory", str(Path(self.root) / ".aws-sam/build"))
                if not isinstance(directory, str):
    ...
    def sam_exec(self) -> str:
        ...
        if not isinstance(self.__sam_exec, str):
            raise TypeError(f"Field `tool.hatch.build.targets.{self.plugin_name}.sam_exec` " "must be a string.")
    ...
    def use_sam(self) -> bool:
        ...
        if not isinstance(self.__use_sam, bool):
            raise TypeError(f"Field `tool.hatch.build.targets.{self.plugin_name}.use-sam` " "must be a boolean.")
    ...
    def sam_params(self) -> Optional[List[str]]:
        ...
        if self.__sam_params is not None and not isinstance(self.__sam_params, list):
            raise TypeError(f"Field `tool.hatch.build.targets.{self.plugin_name}.sam-params` " "must be an array.")
